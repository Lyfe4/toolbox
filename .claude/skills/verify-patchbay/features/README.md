...
| `tools-index` | [tools-index.md](tools-index.md) | `/tools` — browsing, searching and filtering every tool          | yes       |
...
- **Per-tool correctness beyond `base64` and `hash`.** `drive.mjs` proves those two against Node oracles. Every other tool, and `structured-data`'s full format matrix, is covered by the unit suite and `pnpm check:browsers`, not here. Add a drive when one of them changes.
