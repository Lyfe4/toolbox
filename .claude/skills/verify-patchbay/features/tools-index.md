...
- `search` — matches names, summaries **and** keywords, so `sha` finds Hash.
...
- **Enumerate the cards** → `page.locator('a[href^="/tools/"]').evaluateAll(els => [...new Set(els.map(el => el.getAttribute('href')))])` → exactly the ten manifest ids. **Deduplicate**: a card may carry more than one link to the same tool.
...
- **The count of cards is not the count of links.** Cards can contain several anchors to the same tool, so compare a `Set` of hrefs, not `locator.count()`.
...
- **The category select is a Radix combobox, not a native `<select>`.** `selectOption()` does nothing. Click the combobox, then click the `option` role in the popover — the pattern `drive.mjs` uses for `Target format` on tool pages.
...
- **A no-match query needs its positive partner.** "`zzznope` shows nothing" passes just as happily on a search box that has wedged itself empty. `probe-search.mjs` clears the field afterwards and requires all ten cards back.
