...
/*
 * THE GRID IS NOT HERE ANY MORE. Its pitches, its scale cascade and the
 * device-pixel arithmetic that places its rules all live in `grid.ts`, which
 * also carries the reasoning for why it is drawn rather than composited.
 * `GRID` stays here, because it is the snap step first and the grid's pitch
 * second.
 */
