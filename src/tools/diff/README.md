...
themselves are correct as written**.
...
widths, generated by [`scripts/generate-diff-oracle.py`](../../../scripts/generate-diff-oracle.py)
and committed, so the suite needs no git. Two questions are asked of it
separately:

- **Is it spelled the same?** 32 of the 38 are byte-identical. That was 14
  before the oracle existed: the hunk header was written `@@ -2,1 +2,1 @@` where
  `git diff` and GNU `diff` both omit a count of one and write `@@ -2 +2 @@`.
  Both are accepted by `git apply`, so nothing was broken — but no patch this
  tool produced was ever byte-identical to git's, which makes every comparison
...
- **Does it mean the same?** All 38 reproduce the changed file exactly when
  ...
  the additions and this groups each line with its replacement; both of this
...
- **A hunk that touches only one side.** Unified format writes `-N,0` where N is
  ...
  the file. This wrote `@@ -0,0 +7 @@` for an insertion in the middle, which
