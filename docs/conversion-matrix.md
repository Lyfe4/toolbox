...
- [Found in round twelve, by writing the notes the matrix had already promised](#found-in-round-twelve-by-writing-the-notes-the-matrix-had-already-promised)
...
- [A plan for the rounds after this one](#a-plan-for-the-rounds-after-this-one)
...
| **exact, below the token** | The same, where the published vector is not a JWS at all - a key, a message and a signature. Also round six: nothing published is a JWS for HS384, HS512 or ES384, so what is settled is the algorithm table those three depend on and NOT the token handling around it, which the token vectors of the other nine algorithms settle instead.                                                                                                                                                                                                                                                                                             |
...
_Round seventeen:_ the column was empty again, and the sentence above was right
again. The documentation audit found three losses that cells here called
`lossy, told` and nothing told — a byte order mark removed at the regex port, a
Markdown link whose address was refused, and a `null` written to CSV as an empty
cell — each fixed this round rather than moved into the column.
...
| ----- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CSV   | **exact**                               | 32 documents read by CPython's `csv.reader` and by this parser, field for field, including a lone CR terminator, a NUL byte, a quote opening mid-field, a field of four quotes, CRLF inside a quoted cell and every delimiter offered. One divergence, asserted as itself: a blank line between records, which Python yields as an empty row and this parser drops, so a trailing newline and a mid-file blank line behave the same way. [`csv.oracle.test.ts`](../src/tools/structured-data/csv.oracle.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| TSV   | **exact**                               | The same parser with a tab as the delimiter, so the CSV corpus is its evidence for every rule that is not the delimiter. The delimiter itself is exercised by **one** of the 32 cases; this cell said "Same corpus, tab delimiter" until round seventeen, which read as 32.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| JSON/YAML → CSV/TSV           | **lossy, told**          | Three losses, all real: a nested value becomes compact JSON inside the cell, reported **by path** (`$[0].user`); a key absent from one row becomes an empty cell indistinguishable from a present-and-empty one, and the note names the **columns**, not the paths; and every value becomes text. Of the last, a `null` is reported by path from round seventeen — `The null at $[0].b became an empty cell` — because it comes out as the same empty cell. A number or a boolean becoming text is **deliberately not reported**: CSV has no types, the digits are all there, and a note on every export is one nobody reads; the flat-table negative control in `reports.test.ts` holds that silence. Until round seventeen this cell said all three were "reported by path", and the third had no note at all. A non-array, or an array of non-objects, is refused clearly. [`reports.test.ts`](../src/tools/structured-data/reports.test.ts)                                                                 |
...
| Near-JSON: single quotes, unquoted keys, trailing commas                                            | **exact**                                          | Read as the document the author meant. A trailing comma on its own is removed by the JSONC step and reported as `Read as JSONC` ([decision 7](#7-jsonc-is-a-real-step)); single quotes and unquoted keys go through the YAML fallback and say `Read as YAML, not JSON`.                                                                                                                                                                                                                                                  |
| Near-JSON: `//` comments, block comments, a literal newline in a string, a key split over two lines | **fixed this round**                               | Were **broken**: the YAML fallback folded lines and produced a plausible object. A literal newline in a string and a split key are reported as the JSON syntax error they are. The two comment forms were too, in round two; since [decision 7](#7-jsonc-is-a-real-step) they are removed by the JSONC step and the document is read, with a `Read as JSONC` note.                                                                                                                                                       |
...
| A one-column CSV                                                                                    | **refused, stated** / **read, from a file's name** | Round thirteen, SD-1's real gap. It has no delimiter, and "several lines of one field each" is also prose, a log and a word list, so a signal for it would claim everything. Pasted, auto-detect refuses it and the refusal says to choose CSV, which reads it. From round twenty-three a FILE named `.csv` or `.tsv` is read as one, when it is one column under every offered delimiter and nothing in the content says otherwise, and the report says `(from the file name)`. Known limitation 14 in the tool README. |
...
| Whatever was detected                                                                               | **fixed in round three**                           | The tool now says which format it decided on, which delimiter it used, and whether it guessed at all, on a `Detected` report port drawn on `/tools` and shown beside `Detected` in the inspector. It is not what the canvas node prints — a node summarises its first output, and this is the third — though a `warn` note's title reaches the node face. This cell said "summarised on the canvas node" until round seventeen.                                                                                          |
...
_Round twelve: written. Rows 8 and 9 turned, and all three are said on a
`YAML → YAML` run — see that row of the Between formats table._
...
| ----------------------------------------- | -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Markdown → HTML                           | **lossy, told**                                                | CommonMark 0.31.2, 624 of 652, and GFM, 21 of 24, compared by parsed DOM — and **624 of 652 is not what `exact` means**, which is what this cell used to claim. 22 of the 28 differ because this tool will not copy raw HTML through, and that refusal is the product; the other six are URLs — four schemes the list refuses, a scheme's case and a relative URL with a colon. It is now reported: the same chain is run with the allow-list off and the two documents compared, so the note names what was really removed rather than what a schema suggests. **Round seventeen: that sentence was not yet true of a Markdown source.** A link whose address was refused, `[x](javascript:alert(1))`, lost its link with no note, because the comparison only ran on a document containing `<`; an autolink with a refused scheme was told its `<a>` was not on the list; and the note's own body said a README's `<details>` block does not survive, which it does. Each now gets the sentence an HTML source gets (`1 link became plain text`), and the `<details>` clause is gone. This cell said "every one of the 28" until round seventeen. See [Markdown to HTML, relabelled](#markdown-to-html-relabelled).                                                                                                                               |
...
| Markdown → Markdown                       | **lossy, told**                                                | Normalisation through HTML. Three constructs do not survive and each is named: footnotes stop being footnotes (a reference becomes a link to an anchor, the definitions a `## Footnotes` section), `$$…$$` display maths becomes a fence tagged `math`, and a bare URL becomes an explicit link when `linkify` is on. **From round seventeen the allow-list is asked too**: raw HTML it refuses, such as `<foo>bar</foo>`, is named as a `warn`, and a link or image whose address it refuses says what became of it. Until then this target never asked it, and `<foo>bar</foo>` came back as `bar` under a note saying the meaning was unchanged; that note now says "apart from what the note above names" when something was removed. Reformatting — a different bullet, a different heading style — is reported at `info`, because the document means the same thing and a warning on every run is one nobody reads.                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `rendered` port                           | **exact**                                                      | Always sanitised HTML, for every source and target. Asserted for four of the eight pairs, with a document carrying an event handler and a `<script>`: an HTML source on the `html`, `markdown` and `text` targets, and a Markdown source on `markdown`. Until round seventeen this cell said only "Asserted".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `<mark>`, `<kbd>` under Keep the text     | **fixed in round thirteen**                                    | Was **broken** under the default policy, labelled _Keep the text, drop the tag_: `<mark>` became Markdown emphasis and `<kbd>`, `<samp>`, `<var>` became code spans, because the policy registered no handlers and upstream substitutes for eight elements on the list: `mark`; `kbd`, `samp` and `var`; `dl`, `dt` and `dd`; and `q` (this cell said seven until round seventeen). Each now becomes its words and the census names the element that went (corpus row 17, re-specified - see test-findings).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| --------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MD5       | **exact** | RFC 1321 appendix A.5, all seven vectors, plus a chunking property under `fast-check`, and from round seventeen `node:crypto`'s digests at 55, 56, 57, 63, 64, 65, 119 and 120 bytes, either side of a block boundary. Until then that test compared MD5 with itself. This is the one hash implemented here rather than by the platform. |
| SHA-1     | **exact** | The empty input, and FIPS 180-4's `abc` and 448-bit message.                                                                                                                                                                                                                                                                             |
| SHA-256   | **exact** | Same three.                                                                                                                                                                                                                                                                                                                              |
| SHA-384   | **exact** | The empty input and FIPS 180-4's `abc`, added this round — there was a length assertion and nothing else, which every wrong answer of the right size satisfies. From round seventeen also the 896-bit message, which exercises the second 1024-bit block; until then the test's comment claimed a long message it did not have.          |
| SHA-512   | **exact** | Same.                                                                                                                                                                                                                                                                                                                                    |
...
| The whole pipeline, per algorithm                                  | **HS256, RS256, ES256 and PS256 only** | `run` decodes before it verifies and `decodeToken` requires a JSON payload, because RFC 7519 requires a JWT's payload to be one. **Almost every published JOSE example is a JWS and not a JWT**: A.4 signs the ASCII string `Payload`, the cookbook signs a line of Tolkien, and Wycheproof's tokens sign short byte strings - an empty payload, `a`, runs of NUL or 0xFF bytes, `123400`. Five published valid vectors have a payload that parses as JSON - RFC 7515 A.1, A.2 and A.3, and Wycheproof's two PS256 salt cases, whose payload `123400` happens to (58 of Wycheproof's invalid cases share it). One vector for each of those four algorithms - A.1, A.2, A.3 and the first PS256 salt case - runs end to end in Gecko and WebKit through the tool's own worker and verdict banner, with a tampered token, a key of another kind and a different key of the same kind as controls. For the other eight, each engine is asked directly whether it reaches the published verdict under the parameters the fixture records - which is what would catch an engine with no RSA-PSS, no P-384 or no P-521, in which the tool would say `unverified` on a token CI calls verified. |
...
| Expiry, `exp` and `nbf` against now                                | **fixed in round twenty-five**         | Was **broken**: `expired` was decided from `Date.now()` inside the run and cached with it, so a canvas node kept calling a token live after it expired, for as long as the graph was left alone, and the tool page did until Run was pressed again. The output now carries no verdict and the view decides it by the reader's clock as it draws, saying so. Held by `determinism.test.ts` (the same value whenever the tool runs), `jwtValidity.test.tsx` (read hours after the run, with a controlled clock, on a node and on the page, with unexpired tokens read as late as the controls), `validityAt` held at the second either side of `exp` plus the tolerance, and `checkJwtValidity` in two engines.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Unified patch, meaning                    | **exact**       | At the default setting all 36 that have hunks reproduce the changed file exactly when applied - the two `crlf against lf` cases have none, deliberately, as the Line endings row says - and with `Line endings: compare` all 38 do, by a patch applier written for the test and validated against git's own patches first. The four that are spelled differently are the terminator cases, where git groups removals then additions and this groups each line with its replacement; both were given to real `git apply`, which produced the right bytes for each. |
...
| A byte order mark in the subject | **lossy, told** | U+FEFF is invisible, is a character, and sits in front of position 0 — so `^` followed by anything else does not match at the start and there was nothing to say why. Named as a `warn` when nothing matched, which is the case it explains. Only a typed subject can carry one: a dropped file's mark is removed by the decoder, and from round seventeen that is said at `info`, `A byte order mark was removed`, where until then it went in silence. A BOM further into the subject is deliberately not named: it does not explain an anchored pattern failing, and claiming it would be noise. |
...
| EXIF, GPS, colour profile | **lossy, told** for GPS; told on the panel only for the rest | Stripped, and the report says what was removed — which in an app whose pitch is that your data does not move is the note that most needed making. **The level depends on what was there.** A source carrying a GPS location gets a `warn`, `GPS location was removed`, which a canvas node prints; one carrying only EXIF, an ICC colour profile, XMP, IPTC or text comments gets an `info`, `Metadata was removed`, which is drawn on `/tools` and never on a node's face. **That split is deliberate, and its reason has been in the tool's README since round twenty-one** (this cell said no reason was recorded until round twenty-six): nearly every photograph carries EXIF, and a warning on every conversion would teach people to look past the one place a lost location, a dropped frame or a flattened transparency appears. It is the one named exception to the rule that a `warn` is anything that went in and did not come out, and the rule says so where it is stated. Separately, WebKit's encoder writes an ICC profile named `Skia` into every output; that is reported at `info` and left in place. |
...
| Conversion                    | Verdict             | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MKV/MOV/TS/AVI → MP4          | **lossy, told**     | Every coded picture is the encoder's own, byte for byte. From TS, and from an AVI carrying start-code H.264, the framing around each picture is rewritten to MP4's length prefixes and the result says so, at `info`; MKV and MOV already frame their pictures that way, so only the container around them changes and there is no such note.                                                                                                                                                                                                                                                                          |
...
[`src/tools/timestamp`](../src/tools/timestamp). Unix time in four units, RFC
3339 and the ISO 8601 around it, RFC 5322 dates and a readable form, in any
IANA zone, both ways. The instant is a BigInt of nanoseconds, so nothing is a
double at any point; the zone rules are the engine's own `Intl`, which is the
one input this tool does not control - see the last rows.

| Conversion                              | Verdict                                                   | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unix time → a UTC date                  | **exact**                                                 | CPython's `datetime` for 50 instants on a fixed stride across years 1 to 9999 - year, month, day, time, microsecond and ISO weekday - in [`timestamp.oracle.test.ts`](../src/tools/timestamp/timestamp.oracle.test.ts). The nanoseconds below the microsecond are integer arithmetic on the same BigInt, and `1727308800123456789` - past 2^53, and rounded by any `Date` - is asserted to survive to the digit. The fixture's own microsecond counts had to become strings: `253402300799999999` read as a JSON number arrives as `253402300800000000`, and the first version of the test compared against that. |
| RFC 3339 → Unix time                    | **exact**                                                 | RFC 3339 section 5.8's five examples, each held to the instant the RFC's prose says it means - a leap second at two offsets and the Netherlands' +00:20 of 1937 among them.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ISO 8601 → Unix time                    | **exact**, with five named differences                    | 38 strings put to CPython's `datetime.fromisoformat`, an independent ISO 8601 reader, and committed with its answers. The five that differ are a table in the test, each asserted to still differ: a leap second at two offsets (Python has no second 60), a lower-case `t` and `z` (RFC 3339 section 5.6 allows them), an ISO week date and an hour with no minutes (this tool reads neither). A date with no offset is a sixth kind: Python leaves it naive, this reads it in the zone option, and a note says so.                                                                                              |
| RFC 5322 → Unix time                    | **exact**, with three named differences                   | 15 email and HTTP dates put to CPython's `email.utils.parsedate_to_datetime`. The three that differ are deliberate and asserted: a leap second, which Python refuses; a two-digit `60`, which section 4.3 puts in 1960 and Python in 2060; and a weekday the date contradicts, which section 3.3 forbids, Python ignores and this refuses.                                                                                                                                                                                                                                                                        |
| A readable date                         | **exact**                                                 | Written from fixed English names, held to CPython's `calendar` in the C locale, and never by `Intl.DateTimeFormat`, whose output is the engine's CLDR data at the engine's own version. Every notation this tool writes is read back to the nanosecond, by a property over years 1 to 9999 in four zones.                                                                                                                                                                                                                                                                                                         |
| A wall time in a zone, with one instant | **exact**, against IANA's rules                           | CPython's `zoneinfo` at tzdata 2026d: the offset and wall time for 396 instants in 22 zones, and those zones' every offset change from 1900 to 2040, to the second. The tool's arithmetic is run against those rules rather than the machine's, so no unit test depends on the tz database it happens to run beside.                                                                                                                                                                                                                                                                                              |
| A wall time the clocks skipped          | **lossy, told**                                           | PEP 495's `fold` at every clock change in those 22 zones from 1970 to 2030 - 2,460 wall times, inside each gap and overlap and at the edges of every sixth - with `fold=0` as the default rule (RFC 5545's and Temporal's `compatible` too) and the earlier and later instants as the other two. Moved forward by the gap, and said: corpus row 23.                                                                                                                                                                                                                                                               |
| A wall time the clocks showed twice     | **lossy, told**                                           | The same wall times. The first of the two is taken and both are named: corpus row 24.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| A unit read off a number's size         | **lossy, told** outside 1980-2100; **exact** inside it    | Inside that window the four readings of any number are at least 77 times apart, so exactly one lands in it and the digit rule picks it. Outside, the pick is doubted, with the other readings and - for eight digits - the YYYYMMDD date the number also spells: corpus row 21. Choosing the unit turns the note off.                                                                                                                                                                                                                                                                                             |
| A Unix target coarser than the instant  | **lossy, told**                                           | Floored, as POSIX and `Math.floor` do, and said: corpus row 22. The default target is the coarsest unit that holds the instant exactly, so it never fires on a default run, and the Notations output carries every unit as an exact decimal.                                                                                                                                                                                                                                                                                                                                                                      |
| Digits past the nanosecond              | **lossy, told**                                           | Corpus row 26.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| A leap second                           | **lossy, told**                                           | Read as the second after it, which POSIX gives the same number, and said: corpus row 25. Twenty-seven leap seconds, from IANA's `leap-seconds.list`, held to the SHA-1 the file carries for itself. A `:60` that was not one is refused.                                                                                                                                                                                                                                                                                                                                                                          |
| The input's own offset                  | **lossy, told** on the panel only                         | `+02:00` in, a UTC answer or a Unix time out: the instant is exact and the offset is not in the answer. An `info` note rather than a `warn`: the target has no room for an offset by definition, and a warning on every date converted to a number would be the one nobody reads.                                                                                                                                                                                                                                                                                                                                 |
| An offset that is not a whole minute    | **exact**, written outside RFC 3339 and said              | Local mean time is to the second - Africa/Monrovia was -00:44:30 until 1972 - and RFC 3339 has no spelling for it. Written with its seconds, as Python's `isoformat` and Temporal do, with an `info` note, because a rounded offset beside an unrounded wall time names a different instant.                                                                                                                                                                                                                                                                                                                      |
| Out of `Date`'s range                   | **refused**, stated                                       | 100,000,000 days either side of 1970, the range every engine's zone data can be asked about. Both edges are held to the strings `toISOString` writes for them.                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| A named zone's offset, in a browser     | **exact to the browser's own tz release**, and told which | The engine is the reference, and engines differ: Playwright's Gecko answers like tzdata 2026b and its WebKit like 2025a, so for America/Vancouver in January 2027 one says -07:00 and the other -08:00. `checkTimestampZones` holds each engine to the oracle at every zone instant no release from 2022a to 2026d changed, reports the release each engine matches from fifteen sentinels checked against all twenty releases, and holds the tool page's report to naming that release.                                                                                                                          |
| Relative time, "in 3 hours"             | **not built**, decided                                    | A result is cached on its inputs and reproduced by a share link, and a relative time is a statement about now. See the tool's README.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Natural language, "next Friday"         | **refused**, decided                                      | No reference to check a reading against, and a dependence on the moment it is read.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
checked to fail against the old rule, and by checks in
`cross-browser-check.mjs` that put the 800 ms pause in on purpose: two when
this was written, three now.
...
| A `bytes` output into a document port | **one difference, now told**  | A UTF-8 byte order mark is **removed** when bytes are decoded at a document port. Typing the same document into the box keeps it, because nothing decoded anything. It is conventional and almost always wanted, and it is still a byte that went in and did not come out — so every document port now says so. It stays removed; see [What the count was, and is](#what-the-count-was-and-is). |
...
The nested value is named **by path** — `$[0].user` — and the missing keys by
column, each capped at five with a count, because a thousand-row export with one nested column would otherwise
...
that cost nothing, and a node never prints one. One exception is named rather
than left to be found: image metadata other than a location is removed at
`info`, because nearly every photograph has some and a warning on every
conversion is one nobody reads - see [Image](#image). `regex-tester` carries `warn`
...
| A `null` that became an empty cell                                                                   | `output`                                        | The same. Round seventeen.                                                                                                                                                                           |
...
| A JSON key written twice                                                                             | `output`, `data`                                | The **read** half. `JSON.parse` kept one value, and so has `data`.                                                                                                                                   |
| A CSV header cell trimmed                                                                            | `output`, `data`                                | The **read** half. The header becomes the keys.                                                                                                                                                      |
| `Not carried over`: comments, anchors, tags, styles                                                  | `output`, `data`                                | The **read** half. The parsed structure never had them.                                                                                                                                              |
| A TSV cell holding a tab or a line break                                                             | `output`                                        | The write half, like the nested-value note: `data` is untouched.                                                                                                                                     |
| `text-convert`: what the allow-list or the sanitiser removed, a refused link or image                | `output`, `rendered`                            | The sanitised hub is `rendered`, and `output` derives from it.                                                                                                                                       |
| `text-convert`: what the Markdown round trip lost, an identifier renamed or dropped                  | `output`; `rendered` too on the Markdown target | The round trip runs **after** the hub, so `rendered` still has it, except on the Markdown target, where `rendered` is re-rendered from `output`. This row said `output` alone until round seventeen. |
...
| `color-convert`: a colour clipped or clamped                                                         | `output`, `swatch`, `all`                       | The **read** half. Three spellings of one parsed colour, and no port escapes it.                                                                                                                     |
| `timestamp`: a guessed unit, a moved or doubled wall time, a leap second, digits past the nanosecond | `output`, `all`                                 | The instant itself changed, so the answer and every notation of it carry the loss. Missing from this table from round twenty-four to round twenty-six.                                               |
| `timestamp`: a Unix target coarser than the instant                                                  | `output`                                        | The **write** half: `all` has every unit exactly, so it is the way around this one.                                                                                                                  |
...
- `checkLossCorpus` in
  [`cross-browser-check.mjs`](../scripts/cross-browser-check.mjs) is the other
  half of the same definition, and since round twenty-three it reads the SAME
  FILE: every row of the corpus, driven on `/tools` and on a canvas node in
  Firefox and WebKit. On the page, one note drawn with a box of non-zero size
  and no click anywhere, at the Warning level, whose title holds the row's
  subject and whose words hold every `mentions` and every `drawn.says`; on the
  node, a face beginning `Lossy ·`, a `lossy` verdict and `lossy:` in its
  accessible name. Until then five sections listed the rows a second time by
  hand, and the copy had drifted: seventeen rows were on a page, ten on a node,
  and row 3 on neither. A row added to the corpus is driven on the next run
  with no edit to the harness.
- `checkValueModel` in the same file is round eleven's, and it is the one whose
  subject is a **refusal** rather than a note, which no corpus row can hold - a
  document that is refused has not been converted. A refusal has two surfaces
  and they carry different amounts of text: the panel on `/tools` shows the
  message, the code, the line and column, and the detail under them; a canvas
  node shows the **message alone**, because a node has no detail line. So both
  are asked, and the message is held to naming the value model and to not
  naming JSON on a `YAML → YAML` run in either place. Beside it, the
  six-offender enumeration, the target-fitted rounding advice, rows 4 to 7 in
  one document as ONE warning, and the refusals that carry a position.
- `checkColourContrast` holds the colour tool's contrast table - composited
  ratios read off the rendered table - against the engine's own compositor, and
  `checkPastedCensus` holds round ten's correctness fix in a real engine: the
  Markdown for corpus row 14's input, read back out of the box, is three lines
  and every one a table row.
...
about that row's subject may fire on it. Since round twenty-three the sharper
controls the harness used to keep to itself - `"2024": launched`, a header its
author quoted, one key in two sibling objects, a literal block - are in the
corpus too, as `drawn.controls`, and both the unit test and the browser run
every one. A mark that fires on a clean canvas is
...
| JSON/YAML → CSV/TSV                 | **lossy, told**             | Nested values by path, absent keys by column; a `null` by path from round 17 |
...
| Markdown → Markdown                 | **lossy, told**             | Three named constructs, reformatting at `info`; the allow-list from round 17 |
...
**Where a note lives, and where it does not.** Every tool that can lose
something carries its notes on a `report` port, which is what the canvas node
reads, and `resultSummary.test.ts` names them. (This sentence listed them by
name until round twenty-six, and had left `timestamp` out since the day it
shipped with its port - found by listing every paragraph that names several
tools at once, an enumeration no gate reads.) `diff` and `regex-tester` have no report port and
...
arrived. It stays removed; every document port now says so — five ports, on four
tools:
...
- `diff`'s node stops saying a bare `Identical`, and says
  `Identical · BOM differs` instead, because `Identical` is the one summary in the set
...
- `regex-tester` says `A byte order mark was removed` when a file's mark was
  decoded away, and names a mark still in front of position 0 — which only a
  typed subject can carry — as a `warn`, because it is exactly why an anchored
  pattern finds nothing with no explanation. **Until round seventeen only the
  second half was true.** This list said "all four document ports now say so";
  there were five, and the regex port removed the mark in silence, because its
  `run` never asked. The documentation audit found it; `regex.test.ts` now holds
  both halves.
...
Most of it was red when this was written, in round nine, and that was the
point; all twenty rows are green as of round thirteen. A cell may say `lossy, told` only when
...
**The timestamp round added six rows, 21 to 26, and all six were told the day they landed: 26 of 26.** They are the first rows written for a tool as it was built rather than found in one afterwards, which is the only way the ratio stays a measurement of the tools rather than of which rounds happened to look. Two of them carry their zone in the input - `[Europe/Berlin]` - because `drawn.choose` can drive a select and not a text field.

**Round twenty-three moved no verdict: 20 of 20 before, 20 of 20 after.** It
made the corpus the harness's list as well as this test's. Each row now carries
a `drawn` block - the tool page's option labels, the words its warning must
hold, the words no note may, and the harness's sharper controls - which
`scripts/cross-browser-check.mjs` reads to drive the row in two engines and
this test holds to the tools, so a phrase no tool writes fails `pnpm test`
before any browser starts. The verdict is still derived from `expect` alone: a
wrong harness phrase is a failing test, never a row turning silent.
...
**26 of 26** documented losses are told.
...
| 21  | A Unix time's unit read off its size, outside 1980 to 2100                  | Timestamp · A unit read off a number's size        | `timestamp`       | **lossy, told** |
| 22  | Milliseconds written as whole Unix seconds                                  | Timestamp · A Unix target coarser than the instant | `timestamp`       | **lossy, told** |
| 23  | A wall time the clocks skipped, moved forward by the gap                    | Timestamp · A wall time the clocks skipped         | `timestamp`       | **lossy, told** |
| 24  | A wall time the clocks showed twice, read as the first                      | Timestamp · A wall time the clocks showed twice    | `timestamp`       | **lossy, told** |
| 25  | A leap second read as the second after it                                   | Timestamp · A leap second                          | `timestamp`       | **lossy, told** |
| 26  | Digits past the nanosecond dropped                                          | Timestamp · Digits past the nanosecond             | `timestamp`       | **lossy, told** |
...
- **The `report` ports changing what any tool produces.** `output` and `data`
  ...
  _Round seventeen: the value was; the test named for both passed no key, so
  there was no verdict to compare. `jwt.test.ts` › "verifies a genuine
  signature over a claim it had to round" is the verdict half._
...
- **A malformed signature that makes WebCrypto throw.** The `try` in `verify.ts`
  said one does; its comment now says what was measured. An empty, 32-, 63- and 65-byte P-256 signature and an empty and
...
- **A published JWT — not JWS — vector for anything past RFC 7515 A.3.** RFC
  ...
  the only exception found. _Round seventeen: the only valid ones. 58 of
  Wycheproof's invalid cases carry the same payload, which is why the fixture
  records `payloadIsJwtShaped` per case._ If a published JWT vector for PS384, ES512 or any
...
Over 284 documents on both targets, both are zero. **The zeros are in the
gate; the counts behind them are not** — the test asks only that each kind be
named more than twenty times. Re-measured in round seventeen: on a JSON target
44 comments, 30 anchors, 34 tags and 59 block styles are correctly named; on a
YAML target the same 44, 30 and 34, 34 block styles and 25 flow collections.
Until round seventeen this paragraph gave the JSON figures as holding on both
targets. It caught eight of the eighteen deliberate breaks round twelve was
checked against.
...
where a 500 ms stopwatch ran inside each of three hundred cases; the budget went
onto the property, at fifteen times its measured cost - and in round
twenty-six off the clock altogether: every bound in that file is now the bytes
a reader asks its source for or the bits a parameter-set parse consumes, and
three of its stopwatches turned out never to have been able to fail against the
guard they were named for.
...
grid of 64x64 flat blocks — two of them checkerboards, one of one-pixel squares
and one of two-pixel squares, which is where
...
`subtle.verify` said a malformed signature "throws rather than returning false".
...
from a code comment. The comment has since been rewritten to say so.
...
ran first carrying the old assertions. So the harness hashes itself and the one
module it imports, `serve-dist.mjs` (not the fixtures it reads at run time;
this said "everything it loads" until round seventeen), prints the digest at the start so a run can be matched to a
...
splitting, which does not vary by hash and is settled by the token vectors of
the other nine algorithms.
...
string `Payload`. The cookbook signs a line of Tolkien. Wycheproof's tokens sign
short byte strings: an empty payload, `a`, runs of NUL or 0xFF bytes, `123400`.
This said "Wycheproof signs `foo`" until round seventeen; no token in the
fixture does.
...
parse as JSON. One vector for each of those four algorithms, the first salt case
for PS256, runs end to end in Gecko and WebKit through the worker
...
Six rounds in, when this was written, the list was short and every item on it
was short for a stated reason rather than for want of trying. Round six closed the first entry as far
...
_Round seventeen: eight. The same round drove PS256 end to end through
Wycheproof's salt case, as item 1 of the list above says._
...
- **The tools this document has never covered.** Base64, hash, regex and colour
  ...
  _Round seventeen: the hash half was false when it was written. The Hash
  table has cited RFC 1321's and FIPS 180-4's vectors, and the tests used
  them, since round two. What was thin was MD5 across a block boundary and
  SHA-384 and SHA-512 with no second block; both were filled in round
  seventeen._ **This is
...
and the count was the measure. It was **zero** when round seven wrote this (round
eight then found seventeen, and the count became the ratio in
[The corpus, the ratio](#the-corpus-the-ratio-and-why-it-is-not-a-number-any-more)),
and round four's question was the one that replaced it: of everything this document asserts, how much could fail?
