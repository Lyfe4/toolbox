...
| A `<thead>` the serialiser wrote | **corrected in round ten** | The invention note was **false** on the commonest shape of hand-written table there is. `<table><tr><th>h</th></tr>` parses with its row inside an implied `<tbody>` and no `<thead>`, and every table this tool writes has a `<thead>` — so the census saw an element appear and the note said the table had gained an empty header row. It had not. Shipped on `HTML → HTML (normalised)` since round four, and found by extending the census to the Markdown target, where a false note would have been the first thing anybody saw. `<thead>` and `<tbody>` are filtered out of the reported names in `normalisation.ts`, where the sentence is written, rather than in `compareMarkup`, which still answers the question it is asked. A second sentence went the same way: the header-row EXPLANATION was appended to every invention there was, so a `<mark>` becoming `_…_` invented an `<em>` and the note explained it with a fact about tables. It is conditional on `<tr>` or `<th>` now.                                        |
...
| 16  | `class="btn"` emptied to `class=""`                      | Text convert · HTML → HTML (sanitised) | `text-convert`    | **lossy, silent** |
| 17  | `<mark>` and `<kbd>` given formatting they never had     | Text convert · HTML → Markdown         | `text-convert`    | **lossy, silent** |
...
4. **`compareMarkup` against an element that MOVED, and against a VALUE that
   ...
   **The sentence that used to end this item was wrong.** It said the other half
   ...
   not move.
   **A third dimension is what it would take**, and it now has two customers
   rather than one: the same text census that would let a note say `class="btn"`
   became `class=""` is what corpus row 13 needs to name a dropped caption's
   CONTENTS rather than its tag.
