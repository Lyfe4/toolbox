...
| EXIF, GPS, colour profile | **lossy, told** for GPS; told on the panel only for the rest | Stripped, and the report says what was removed — which in an app whose pitch is that your data does not move is the note that most needed making. **The level depends on what was there.** A source carrying a GPS location gets a `warn`, `GPS location was removed`, which a canvas node prints; one carrying only EXIF, an ICC colour profile, XMP, IPTC or text comments gets an `info`, `Metadata was removed`, which is drawn on `/tools` and never on a node's face. No reason for that split is recorded. Separately, WebKit's encoder writes an ICC profile named `Skia` into every output; that is reported at `info` and left in place.                                                                                            |
...
`text-convert`, `base64`, `jwt-decode`, `color-convert` (from round nine),
`image-convert` and `video-remux` carry theirs on a `report` port, which is what
the canvas node reads; `resultSummary.test.ts` holds that list. `diff` and `regex-tester` have no report port and
