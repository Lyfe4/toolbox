...
- `checkColourReports` in
  ...
  sentence on a canvas node's face, the word `lossy:` in its accessible name,
  and the contrast
...
- `checkMarkdownCensus` in the same file is round ten's, for the target that
  ...
  size and no click anywhere, the same sentence on a canvas node's face, the
  word `lossy:` in its accessible name, and — because this round fixed a correctness bug as well
