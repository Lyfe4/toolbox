...
| It costs the initial JavaScript payload nothing  | `bundle:check` reports the same figure it did before — the delta is 0 bytes of JS |
...
[docs/conversion-matrix.md](docs/conversion-matrix.md) is the map: every tool,
every direction, every canvas link type, each marked **exact**, **lossy but
told**, **lossy and silent**, **broken** or **not verified**, with what the
verdict rests on. It exists because a converter fails by handing back a
document that looks exactly like the one you asked for, and that failure is
invisible to a green suite when the expected values were written by reading the
code and agreeing with it.
...
The colour row is the one to read carefully. The committed test walks a fixed
stride of 166,112 colours, not the whole cube; **all** 16,777,216 were swept
offline, once, to choose the default precision and to establish that the
stride sees the failures. The sweep is the calibration, and the stride is the
test.

**What that found, in one pass:** a Markdown sentence about HTML detected as
HTML with confidence; a `//` comment folded into the key after it; three lines
of prose returned as one line with the breaks replaced by spaces; a Markdown
hard break rendered as a blank line; `hsl(217 91 60)` converted to white;
13,626 colours that did not survive `oklch()` at the default precision; a
README claiming a test that did not exist. Not one of them was failing a test.
...
The clipboard failures above had a mechanism in the harness. This one looked
exactly the same and had a mechanism in the app.

Under CPU load, in WebKit, a runaway regex node took an unrelated `base64` node
down with it: `error` on a node nobody had touched, `upstream` on the node
downstream of that. Idle, it passed. The convenient reading was "the harness is
sensitive to load", and the reading that mattered was that a person on a slow
machine would see the same thing.

**Neither was established by re-running, so it was measured instead.** The
scenario was driven 48 times across three load levels in both engines, with
every message on the worker port recorded. What that showed was not about slow
machines at all:

| Condition                                    | Bystander            | What was posted to a worker         |
| -------------------------------------------- | -------------------- | ----------------------------------- |
| Idle, two edits as fast as a driver can type | `ok` in ~2.4 s       | regex, base64, base64               |
| 16 busy processes on 16 cores                | `error` 7 of 16      | regex, regex, base64, regex, base64 |
| **Idle, 800 ms between the two edits**       | **`error` 10 of 10** | regex, regex, base64, regex, base64 |

The deciding variable is the gap between two edits, against the pipeline's
300 ms debounce — **whether a person types into one node and then into
another**. Load only widens the gap. On an idle machine, with an 800 ms pause,
it failed every single time in both engines.

The mechanism, read off the recording rather than off the code: the second edit
cancels the first run, and a cancelled run is deliberately not cached, so the
new run **re-posts the runaway**. Two copies of it are now queued. The first
copy's deadline destroys the worker and the base64 request is replayed; the
replayed runaway wedges the next worker, and its death finds the base64 request
out of replay budget and fails it. The worker had never sent a `started` for
that request — it had not run one instruction.

**So the budget was asking the wrong question.** It capped replays to stop a
poisonous input looping; a request that never started cannot be the poison. It
counts starts now, and a request that has never run keeps its budget however
many times a neighbour destroys the worker underneath it. Ten of ten `ok`
afterwards, in both engines, in 3.7 s — and `check:browsers` now puts that
