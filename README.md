...
3,219 tests across 115 files. The count is not the interesting part; what the
tests caught is.
...
| Conversion               | Held to                                                      |
| ------------------------ | ------------------------------------------------------------ |
| CSV, reading and writing | CPython's `csv` module, 32 documents and 12 record sets      |
...
| OKLCH round-tripping     | all 16,777,216 sRGB colours, swept                           |

Each fixture is generated by a script in [`scripts/`](scripts) and checked in,
so nothing shells out at test time and a change to either side is a diff in
review.
